"""AI code reviewer otomatis untuk tiap PR — jalan lewat
.github/workflows/ai-review.yml. Ambil diff PR via `gh pr diff`, baca
AGENTS.md sebagai konteks aturan repo, kirim ke endpoint OpenAI-compatible,
dan posting hasilnya sebagai komentar PR. Exit code 1 (gagalkan job) kalau
review menemukan isu kritis, supaya jadi blocking gate — bukan cuma
informational comment.

Butuh secret `OPENAI_API_KEY` (+ opsional `OPENAI_BASE_URL`,
`OPENAI_MODEL`) di GitHub repo settings. Tanpa `OPENAI_API_KEY`, script
ini posting komentar fallback (bukan review asli) dan tidak menggagalkan
job — supaya PR tetap bisa di-merge sebelum secret itu dikonfigurasi.
"""
...
from openai_compat import call_chat_completions
...
def get_pr_diff(pr_number: str) -> str:
    ...
    # Filter lockfile yang membengkak dari diff supaya tidak menghabiskan
    # konteks yang seharusnya dipakai buat review kode sungguhan.
...
def get_agents_md() -> str:
    try:
        ...
        return "(AGENTS.md tidak ditemukan di root repo)"


def build_messages(agents_md: str, diff: str, repo: str, pr_number: str) -> list[dict]:
    system = (
        "WAJIB HARUS DAN SELALU MEMBALAS DALAM BAHASA INDONESIA CASUAL DAN PROFESIONAL. "
...
def check_critical_issues(review_text: str) -> tuple[bool, str]:
    match = re.search(r"##\s*🚨\s*Isu Kritis[^\n]*\n(.*?)(?=\n##\s|\Z)", review_text, re.DOTALL | re.IGNORECASE)
    if not match:
        return False, ""
...
def write_step_summary(body: str, has_critical: bool, critical_detail: str) -> None:
    ...
    try:
        with open(summary_file, "a", encoding="utf-8") as f:
            ...
            if has_critical:
                ...
                f.write("Silakan perbaiki isu kritis yang dilaporkan di atas sebelum merge.\n\n")
            else:
...
def main() -> int:
    ...
    if not pr_number:
        print("PR_NUMBER tidak diset.", file=sys.stderr)
        return 1

    if not api_key:
        print("OPENAI_API_KEY belum diset — melewati review LLM asli, posting fallback.", file=sys.stderr)
        fallback_review = (
            f"## AI PR Reviewer\n- PR #{pr_number} dicek terhadap panduan `AGENTS.md`.\n"
            "- **Catatan**: set secret `OPENAI_API_KEY` di GitHub repo settings untuk mengaktifkan "
            "review LLM sungguhan (lihat AGENTS.md bagian 6)."
        )
    ...
    print("AI Code Review selesai: bebas dari isu kritis.")
    return 0
